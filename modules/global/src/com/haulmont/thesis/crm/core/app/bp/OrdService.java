...
public class OrdService implements IOrdService {
    ...
    private boolean one()
    {
        try {
            ...
            String Id1C = work1C.setOrd(xdto());
    ...
    private Doc xdto()
    {
        ...
        OrdDoc row = work1C.buildQuery(OrdDoc.class, params);
        ...
        */
        ...
        if (row.getContract() != null) {
            ...
            builderContract.append("&");
            builderContract.append(String.format("офр-%s", row.getNumber()));
            builderContract.append("&");
            builderContract.append(row.getContract().getDocCategory().getCode());
        }

        String currencyCode = work1C.validate(row.getCurrency()) ? row.getCurrency().getCode() : "";
        //************************
        Doc doc = new Doc();
        doc.setRefOrd(work1C.getNullToEmpty(ref));
        //doc.setRefInv();
        //doc.setRefAct();
        ...
        doc.setDescription(work1C.getNullToEmpty(row.getDescription()));
        doc.setTotalSum(row.getFullSum());
        //doc.setIsPosted();
        doc.setIsDel(row.getIntegrationResolver().getDel() != null ? row.getIntegrationResolver().getDel() : false);
        //строки
        for (OrderDetail rowd : row.getOrderDetails()) {
            // 2 - поиск соответсвий (маппинг)
            //************************
