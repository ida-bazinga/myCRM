...
public class CampaignKindBrowser extends AbstractLookup {
    @Named("campaignKindsDs")
    protected CollectionDatasource<CampaignKind, UUID> mainDs;
    @Named("campaignKindsTable")
    ...
    public void init(Map<String, Object> params) {
        ...
        if (mainDs != null) {
            mainDs.addListener(new CollectionDsListenerAdapter<CampaignKind>() {
                @SuppressWarnings("unchecked")
                @Override
                public void itemChanged(Datasource ds, CampaignKind prevItem, CampaignKind item) {
                    if (item != null) {
                        paramsMap.put("ckClass", "%" + item.getEntityType() + "%");
                    }
    ...
    public void OpenProcsEditor() {
        openDialogEditorWithAlias("campaignKindProcs");
    }

    public void OpenReportsEditor() {
        openDialogEditorWithAlias("campaignKindReports");
    }

    protected void openDialogEditorWithAlias(String windowAlias) {
        CampaignKind kind = mainTable.getSingleSelected();
        if (kind != null) {
            ...
            window.addListener(new Window.CloseListener() {
                ...
                public void windowClosed(String actionId) {
                    if (Window.COMMIT_ACTION_ID.equals(actionId)) {
                        CampaignKind item = (CampaignKind) ((Editor) window).getItem();
    ...
    protected void changeKind() {
        mainTable.addAction(new AbstractAction(CHANGE_ACTION_ID) {
            @Override
            public void actionPerform(Component component) {
                CampaignKind selectedKind = mainTable.getSingleSelected();
                Long countEntities = getCountEntities();
                if (countEntities > 0) {
    ...
    protected Long getCountEntities() {
        return mainTable.getSingleSelected() != null
                ? campaignService.getCountWithSpecifiedKind((UUID) mainTable.getSingleSelected().getId())
                : 0L;
    }
    ...
    protected String getMessageInClass(String key) {
        return messages.getMessage(CampaignKindBrowser.class, key);
    }

    protected String formatMessage(String key, Object... params) {
        return messages.formatMessage(CampaignKindBrowser.class, key, params);
    }

    protected class ChangeKindAction extends AbstractAction {

        private static final String KIND_LOOKUP = "crm$CampaignKind.browse";
        ...
        private CampaignKind selectedKind;

        protected ChangeKindAction(CampaignKind selectedKind) {
            ...
            getFrame().openLookup(KIND_LOOKUP, new Window.Lookup.Handler() {
                public void handleLookup(Collection items) {
                    if (items != null && items.size() > 0) {
                        CampaignKind kind = (CampaignKind) items.iterator().next();
                        changeKind(kind);
                    }
                }
        ...
        private void changeKind(CampaignKind kind) {
            List<BaseCampaign> entities = campaignService.getWithKind(selectedKind.getId());
            entityIterator(kind, entities);
            showMsgAfterChange(kind);
        }

        private void entityIterator(CampaignKind kind, List<BaseCampaign> entities) {
            int i = 0;
            int j = 0;
            List<BaseCampaign> entitiesToCommit = new ArrayList<>();
            for (BaseCampaign entity : entities) {
                entity.setKind(kind);
        ...
        private void commitKind(int i, int j, List<BaseCampaign> entitiesToCommit) {
        ...
        private void showMsgAfterChange(CampaignKind kind) {
